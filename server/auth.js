...
const debug = require('debug')(`${app.name}:auth`) // env DEBUG=app.name:auth
...
passport.serializeUser((user, done) => { // passport gives us done
  debug('serialize is happening user.id=', user.id)
  done(null, user.id) // with express-session this was added to the session store -- cookie-session should be added to cookie itself
})

passport.deserializeUser(
  (id, done) => { // id is given to us by passport through its ability to hook into req.session
    debug('will deserialize user.id=%d', id)
    User.findById(id)
      .then(user => {
        if (!user) debug('deserialize retrieved null user for id=%d', id)
        else debug('deserialize did ok user.id=%d', id)
        done(null, user) // creates req.user == logged in user
      })
      .catch(err => {
      ...
      .then(user => {
        ...
        return user.authenticate(password)
          .then(ok => {
            ...
            done(null, user) // calls serialize user with entire user instance
          })
      })
      .catch(done)
  }
